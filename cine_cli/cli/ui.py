...
logger = LoggerAdapter(cine_cli_logger, prefix = Colours.PURPLE.apply("prompt"))

class CineCliTheme(Default):
    def __init__(self):
        super().__init__()
...
# without losing performance is serious business at cine-cli. ~ Goldy 2024
...
def prompt(
    ...
    previous_logger_level = cine_cli_logger.level
    cine_cli_logger.setLevel(logging.CRITICAL)
    ...
    # restore the logger
    cine_cli_logger.setLevel(previous_logger_level)
...
def welcome_msg(
    ...
    now = datetime.now()
    cine_cli_path = Path(os.path.split(__file__)[0])
    adjective = random.choice(
        ("gorgeous", "wonderful", "beautiful", "magnificent")
    )

    random_tips_path = cine_cli_path.joinpath("random_tips.json")
    ...
    if display_tip and display_version is False:
        ...
        if random.randint(0, 1) == 0:
            text += f"\n\n- {Colours.BLUE}Hint: {Colours.RESET}cine-cli {Colours.PINK_GREY}-s films {Colours.ORANGE}mr.robot{Colours.RESET}" \
                f"\n- {Colours.BLUE}Hint: {Colours.RESET}cine-cli {Colours.PINK_GREY}-s anime {Colours.ORANGE}chuunibyou demo take on me{Colours.RESET}"
    ...
    if display_version is True:
        text += f"\n\n{Colours.CLAY}-> {Colours.RESET}Version: {Colours.BLUE}{cine_cli.__version__}{Colours.RESET}"

    if check_for_updates:
        cache = Cache(platform, section = "update_checker")

        if update_available(cache):
            update = update_command(cine_cli_path)
        ...
        if plugin_needs_updating:
            update = update_command(cine_cli_path, plugins_to_update)
