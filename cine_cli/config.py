...
logger = LoggerAdapter(cine_cli_logger, prefix = "Config")

class Config():
    """Class that wraps the cine-cli configuration file. Mostly used under the CLI interface."""
    def __init__(self, override_config: ConfigData = None, config_path: Path = None) -> None:
        ...
        if override_config is None:
            ...
            try:
                self.data = toml.load(self.config_path).get("cine-cli", {})
    ...
    def plugins(self) -> Dict[str, str]:
        return self.data.get("plugins", {"tmdb": "cine-cli"})
    ...
    def get_env_config(self) -> AutoConfig:
        """Returns python decouple config object for cine-cli's appdata .env file."""
    ...
    def __get_env_file(self) -> Path:
        """Function that returns the path to the cine-cli .env file."""
        platform = what_platform()
